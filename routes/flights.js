...
router.post('/create', verifyToken, async (req, res) => {
  try {
    ...
    const newFlight = new Flights({ flight_id, plane_id, departure_airport, arrival_airport, departure_time, arrival_time, seats_total, seats_booked, ticket_price, user});
...
router.post('/booking', verifyToken, async (req, res) => {
    try{
        ...
        const {flight_id, passport_id} = req_body;
        const flight = await Flights.find({"flight_id": flight_id});
